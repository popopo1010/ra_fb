...
| 機能 | 説明 | 経路 |
|------|------|------|
| RA FB 生成 | 初回架電の文字起こしからFB生成 | Slack /rafb_call、ファイルアップロード、CLI、Webhook |
| CA FB 生成 | 法人面談の文字起こしからFB生成 | Slack /rafb_mtg、CLI、Webhook |
...
├── scripts/
│   ├── slack_server.py         # メイン起動（/rafb_call, /rafb_mtg, ファイルアップロード）
...
│   └── 営業のヒアリング情報/     # 営業ヒアリング・構造マップ・訴求ポイント（電気施工管理等）
...
| HEARING_DIR | references/営業のヒアリング情報/ |
...
| 起動 | [起動ガイド](起動ガイド.md) |
| Slack | [テキストファイルアップロード](Slack_テキストファイルアップロード設定.md)、[個別連携](Slack_個別連携設定.md)、[/rafb_call・/rafb_mtg コマンド](Slack_rafb_fb_コマンド設定.md) |
...
| 営業ヒアリング | [README](../references/営業のヒアリング情報/README.md) … 構造マップ・訴求ポイント |
